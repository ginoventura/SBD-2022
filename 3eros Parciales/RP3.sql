...
--1
--a. Consulta que devuelva los grupos que tienen más alumnos que el cupo máximo

select ag.cod_curso, COUNT(*)as inscriptos_confirmados, c.cupo_grupo
	from alumnos_grupos ag
		join cursos c
			on ag.cod_curso = c.cod_curso
  where ag.confirmado = 'S' 
  group by ag.cod_curso, c.cupo_grupo
...
as
begin
	if exists (select ag.cod_curso, COUNT(ag.nro_alumno)as inscriptos_confirmados, c.cupo_grupo
					from alumnos_grupos ag
						join cursos c
							on ag.cod_curso = c.cod_curso
				  where ag.confirmado = 'S' 
...
as
begin
	if UPDATE(cupo_grupo)
	begin
		raiserror('No se permite actualizar el cupo tptal del curso',16,1)
		rollback
	end
end
