...
where c.cupo_grupo < (select COUNT(*)
					  ...
					  where ag.confirmado = 'S'
					    and ag.cod_curso = g.cod_curso
						and ag.nro_grupo = g.nro_grupo)

-- SOBRA LA TABLA GRUPOS

/*
--Pruebas
group by g.nro_grupo, g.cod_curso, c.cod_curso
having (COUNT(ag.nro_alumno)) < c.cupo_grupo
...
---------------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------------
--grupos						----						----						Nada. Salta FK
---------------------------------------------------------------------------------------------------------------------------------
--cursos						----						----						Controlar cupo_total
---------------------------------------------------------------------------------------------------------------------------------
--alumnos_grupos				Controlar cupo_total		---- Libera cupos			Controlar cupo_total
---------------------------------------------------------------------------------------------------------------------------------

create trigger tu_cursos
on cursos
for update
as
begin
	if exists(select *
			...
			where c.cupo_grupo < (select COUNT(*)
								  ...
								  where ag.confirmado = 'S'
									...
									and ag.nro_grupo = g.nro_grupo))
	begin
		raiserror('Se alcanzó el límite del cupo.', 16, 1)
		rollback
	end
...
-- DEBIERA CONTROLAR SOLO LOS CURSOS MODIFICADOS

create trigger ti_alumnos_grupos
on alumnos_grupos
for insert
as
begin
...
-- DEBIERA CONTROLAR SOLO LOS GRUPOS DE LOS ALUMNOS INSERTADOS

create trigger tu_alumnos_grupos
on alumnos_grupos
for update
as
begin
...
-- DEBIERA CONTROLAR SOLO LOS GRUPOS DE LOS ALUMNOS MODIFICADOS
...
create or alter procedure disponibilidad_cupos
(
	@hora_minima			time,
	@cant_max_dias_semana	integer
)
as
begin
	/*select *
	...
	where c.cupo_grupo > (select COUNT(*)
									from alumnos_grupos ag
									where ag.cod_curso = g.cod_curso
									  and ag.nro_grupo = g.nro_grupo)
	  and hg.hora_inicio >=  @hora_minima*/
	
	--cantidad_dias_semana no puede ser mayor a @cant_max_dias_semana

	declare		@cod_curso				char(4),
				@nro_grupo				tinyint,
				@hora_inicio			time,
				
				@cod_curso_proc			char(4),
				@nro_grupo_proc			tinyint,
				@hora_inicio_proc		time,
				
				@cant_dias_semana		smallint, --contador de dias por cod_curso-nro_grupo
				@hora_inicio_minima		time,
				@sirve					tinyint
	...
	select g.cod_curso, g.nro_grupo, hg.hora_inicio
		from grupos g
			join cursos c
				on g.cod_curso = c.cod_curso
					join horarios_grupos hg
	...
	where c.cupo_grupo > (select COUNT(*)
	  ...
	  open cur
	  fetch cur into @cod_curso, @nro_grupo, @hora_inicio
	  
	  while @@FETCH_STATUS = 0
	  begin
		  set @cod_curso_proc = @cod_curso
		  set @nro_grupo_proc = @nro_grupo
		  set @cant_dias_semana = 0
		  set @hora_inicio_minima = @hora_inicio
		  set @sirve = 1

		  while @@FETCH_STATUS = 0 and @cod_curso = @cod_curso_proc and @nro_grupo = @nro_grupo_proc
		  begin
			set @cant_dias_semana = @cant_dias_semana + 1

			if @hora_inicio_minima < @hora_minima
			begin
				--Ya no nos sirve este registro cod_curso-nro_grupo
				set @sirve = 0
			end
		  ...
		  if @cant_dias_semana <= @cant_max_dias_semana and @sirve = 1
		  begin
			--Lo agregamos a la tabla de resultados
			insert into @resultados (cod_curso, nro_grupo)
			values (@cod_curso_proc, @nro_grupo_proc)
		  end
...
--Para probar:
execute disponibilidad_cupos @hora_minima = '09:00:00.0000000', @cant_max_dias_semana = 2
go

-- DEBIERA CONTROLA LOS DIAS DISTINTOS POR SEMANA PORQUE PODRÍAMOS TENER DOS HORARIOS EL MISMO DÍA
-- PUNTOS: 50
