...
--si son de sexo masculino, una corbata. La secretaria de la Gerencia necesita saber cuántos 
--ramos de rosas y cuántas corbatas debe comprar para el mes de mayo.
...
--5) Además, si el empleado cumple 5,10,15,20,25,30... años de servicio, se le regala una placa 
...
--los empleados que ingresaron en el mes de abril para encargar dichas placas.
 select nombre,datepart(year,fechaingreso) as añoingreso,
  datepart(year,getdate())-datepart(year,fechaingreso) as AñosdeServicio, placa=
   case (datepart(year,getdate())-datepart(year,fechaingreso)) % 5
     when 0 then 'Si'  --si la cantidad de años es divisible por 5
     else 'No'
   end
  from empleados
...
--todos los empleados.
select e.nombre, e.cantidadhijos, e.sueldo as sueldobasico, sueldoporhijo =
	case 
			when e.sueldo <= 500 then '200'
		else '100'
	end,
	salariofamiliar =
	case 
			when e.sueldo <= 500 then cantidadhijos*200
		else cantidadhijos*100
	end,
	sueldofinal =
	case
			when e.sueldo <= 500 then e.sueldo+(e.cantidadhijos*200)
		else e.sueldo+(e.cantidadhijos*100)
	end
 		from empleados e
		where e.cantidadhijos > 0
	order by sueldofinal
