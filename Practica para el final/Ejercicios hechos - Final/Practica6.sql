...
-- EJERCICIO 1-A) 
------------------------------------------------------------------------------------------------------------------
-- REGLA DE INTEGRIDAD: el personal asignado a cada servicio debe pertenecer al area responsable del servicio
------------------------------------------------------------------------------------------------------------------
-- Personal de cada servicio que no pertenece al area responsable del servicio
------------------------------------------------------------------------------------------------------------------
	...
	select * 
		...
		where s.nro_area_responsable != ps.nro_area
...
--|----------------------|-------------------|-------------------|------------------|
--| PERSONAL_SERVICIOS	 |		  SI	     |		  NO 		 |		  SI		|
--|----------------------|-------------------|-------------------|------------------|
--| SERVICIOS			 | 		  NO 		 |		  NO		 |		  SI		|
--|----------------------|-------------------|-------------------|------------------|
-- Insert en personal_servicios: si se realiza un insert en personal_servicios, se debe controlar que el
-- personal que se inserta debe pertenecer al area responsable del servicio.
-- Delete en personal_servicios: si se borra un personal_servicios, no deja de cumplir una RI algo que no 
-- existe
-- Update en personal_servicios: si se realiza un update, se debe controlar que el personal (si se actualiza)
-- pertenezca al nro_area_responsable del servicio
---------------------------------------------------------------------------------------------------------------
-- Insert en servicios: si se inserta un servicio, no afecta la RI por que todavia no tendria personal asignado.
-- Delete en servicios: se borra un servicio, no afectaria la RI.
-- Update en servicios: si se actualiza un servicio, afectaria la RI, por lo tanto no se debe permitir actualizar
-- un servicio.
---------------------------------------------------------------------------------------------------------------

-- EJERCICIO 1-C) 
-- Trigger insert/update en personal_servicios
if OBJECT_ID('tri_inup_personal_servicios') is not null
	drop trigger tri_inup_personal_servicios
go

create or alter trigger tri_inup_personal_servicios
	...
	begin 
		if exists (select * 
						...
						where s.nro_area_responsable != ps.nro_area)
		begin
			raiserror('El personal designado al servicio debe pertenecer al arear responsable del servicio', 16,1)
			rollback
		end
	end
go

-- Trigger update en servicios
create or alter trigger tri_up_servicios 
	on servicios
	for update
	as 
	begin
		if update(nro_servicio) or update(nro_area_responsable)
		begin
			raiserror('No se puede modificar el numero de servicio o el area responsable de un servicio', 16, 1)
		end
	end
go
...
create or alter procedure pa_actualizar_solicitudes
as 
	begin
	declare		--Variables de Solicitud
				@año_solicitud_s			smallint,			
				...
				--Variables de ordenes de trabajo
				...
				--Variables que solicita el procedimiento
				@maxima_fecha_fin			date,
				@horas_totales_ordenes		integer

	declare cur cursor for																		--declaramos el cursor
		select s.año_solicitud, s.nro_solicitud, ot.fecha_fin, ot.horas_totales					--para determinados datos
			from solicitudes s
				join ordenes_trabajo ot
					on s.año_solicitud = ot.año_solicitud
				   and s.nro_solicitud = ot.nro_solicitud
			where ot.estado = 'F' or ot.estado = 'A'

		open cur																				--se abre el cursor
			fetch cur into @año_solicitud_s, @nro_solicitud_s, @fecha_fin_ot, @horas_totales_ot
		while @@FETCH_STATUS = 0
			begin
				set @maxima_fecha_fin = @fecha_fin_ot			--se declaran las variables para poder comparar y recorrer
				set @horas_totales_ordenes = 0
				set @nro_solicitud_ot = @nro_solicitud_s
				set @año_solicitud_ot = @año_solicitud_ot

			while @@FETCH_STATUS = 0 and @nro_solicitud_ot = @nro_solicitud_s and @año_solicitud_ot = @año_solicitud_s
				begin 
					if(@maxima_fecha_fin < @fecha_fin_ot)			--si la maxima fecha_fin es menor a la fecha_fin_ot 
					begin
						set @maxima_fecha_fin = @fecha_fin_ot		--se actualiza el valor de la maxima fecha
					end
				set @horas_totales_ordenes = @horas_totales_ordenes + isnull(@horas_totales_ot, 0) --se calcula la hora total de la orden

			fetch cur into @año_solicitud_s, @nro_solicitud_s, @fecha_fin_ot, @horas_totales_ot
		end

		update solicitudes 
			set fecha_entrega = @maxima_fecha_fin,					--se escribe la fecha de entrega que sea igual a la maxima_fecha_fin
				horas_totales = @horas_totales_ordenes				--se actualizan las horas totales de la solicitud
		  where nro_solicitud = @nro_solicitud_ot and año_solicitud = @año_solicitud_ot		--donde el nro_solicitud = nro_solicitud_de la orden y el año solicitud es igual al año de la orden
		end
		close cur
		deallocate cur
	end
