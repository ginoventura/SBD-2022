...
-- Programar una consulta que muestre todos créditos que tienen cuotas vencidas y no pagadas
-- Datos a mostrar: (nro_credito, total_cuotas, saldo_vencido_cuotas)

select c.nro_credito, count(*) as total_cuotas, sum(c.importe) as saldo_vencido_cuotas
			from cuotas c
...
-- Mostrar todas la cuotas vencidas y que aun no fueron pagadas completamente 
-- Datos a mostrar: (nro_credito, nro_cuota, fecha_vencimiento, importe, saldo)
