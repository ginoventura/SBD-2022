...
create or alter procedure get_movimientos_cta_cte
(
	@nro_cliente	smallint,
	@fecha_desde	date,
	@fecha_hasta	date
)
as
begin
	declare @saldo_total	decimal(12,2),
			@saldo			decimal(12,2),
			@debe			decimal(12,2),
			@haber			decimal(12,2),
	...
	select @saldo_total = sum(case when cc.tipo_comprobante in ('FA','ND') then cc.importe_total else -cc.importe_total end)
			from cuenta_corriente cc
		where nro_cliente = @nro_cliente
		and cc.fecha_comprobante < @fecha_desde

	insert into @movimientos (fecha,comprobante,saldo)
		values	(dateadd(dd,-1,@fecha_desde), 'saldo inicial', @saldo_total)


	insert into @movimientos(fecha,comprobante,debe,haber,saldo)
	select fecha = cc.fecha_comprobante,-- convert(varchar, cc.fecha_comprobante,103),
		   comprobante = cc.tipo_comprobante + '-' + format(cc.nro_comprobante,'00000000'),
		   debe = case when cc.tipo_comprobante in ('FA','ND') then cc.importe_total else null end,
		   haber = case when cc.tipo_comprobante in ('RE','NC') then cc.importe_total else null end,
		   saldo = 0.00
			from cuenta_corriente cc
		where cc.nro_cliente = @nro_cliente
			 and cc.fecha_comprobante between @fecha_desde and @fecha_hasta
		order by fecha, comprobante asc
	
	declare c cursor for
	select debe, haber, comprobante
			from @movimientos c
		order by c.fecha,c.comprobante
	open c
	fetch c into @debe,@haber, @comprobante
		while @@fetch_status = 0
			begin

				set @saldo_total = @saldo_total + isnull(@debe,0.00) - isnull(@haber,0.00)

				update c
				set saldo = @saldo_total 
					from @movimientos c
				  where c.comprobante = @comprobante
	...
	select fecha		= m.fecha,
		   comprobante  = m.comprobante,
		   debe			= m.debe,
		   haber		= m.haber,
		   saldo		= m.saldo
		from @movimientos m	
	order by m.fecha, m.comprobante
end


execute get_movimientos_cta_cte @nro_cliente = 1, @fecha_desde = '1998-02-01', @fecha_hasta = '1998-04-30'
