/* -----------------------------------------------------------------------------------------------------------------------------------
   INGENIERÍA INFORMÁTICA – SISTEMAS DE BASES DE DATOS - EVALUACIÓN PARCIAL RECUPERATORIA Nº 2 - 03/07/2020
...
/*
EJERCICIOS:
Ha surgido un nuevo requerimiento en el sistema de cursos de capacitación: "Cada grupo tendrá un único docente". 
...
a. Debe elaborar un script para cambiar la base de datos con las operaciones necesarias para dar soporte al nuevo requerimiento,
   incluyendo la migración de los datos actuales a la nueva estructura. 
   ...
   orden de las operaciones puede ser crítico. Por ejemplo, no se puede eliminar una columna si tiene una regla de integridad. 
   Primero deberá eliminar la regla de integridad.
   Suponga que los grupos registrados al momento en la base de datos tienen un único docente asignado y que, por lo tanto, 
   datos pueden ser migrados sin problemas.
   NOTA1: La migración no debe basarse en operaciones fijas (que usan constantes con los datos actuales), sino que debe estar 
...
c. Utilizando la base de datos original (SIN los cambios implementados), programar una consulta que muestre los alumnos inscriptos
   en grupos de cursos, que aún no tienen docente designado y tampoco los horarios definidos.
