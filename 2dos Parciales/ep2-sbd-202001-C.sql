...
/*
EJERCICIOS:
   a. Ha surgido un nuevo requerimiento en el sistema de gestión de solicitudes de servicios: Teniendo en cuenta que hay un único responsable 
      en cada servicio, se decide cambiar el modelo de datos y registrar el responsable del servicio en la misma tabla servicios.
   ...
   b. Debe migrar los datos actuales de responsables de servicios.
	  NOTA1: La migración no debe basarse en operaciones fijas (que usan constantes con los datos actuales), sino que debe estar programada para 
   ...
   c. Mostrar para cada área, las horas trabajadas por todo su personal, por cada año, entre el 2017 y el 2019, teniendo en cuenta solo las
      órdenes de trabajo finalizadas. El resultado debe estar completo, es decir, si un área en un año no tiene horas trabajadas, entonces 
	  mostrar 0.
...
add responsable	char(1)	 null
go

alter table servicios
add nro_personal	smallint	 null
go

alter table servicios
add foreign key(nro_area_responsable,nro_personal) references personal_areas
go

--MIGRAR DATOS

alter table personal_servicios
drop constraint CK__personal_servicios_responsable__END
...
update s
	set responsable  = ps.responsable,
		nro_personal = ps.nro_personal
--	select s.*,ps.nro_personal,ps.responsable
		from servicios s
...
alter table servicios
alter column nro_personal	smallint	not null
go
...
select ot.nro_area, a.area, ot.año_solicitud as año, sum(ot.horas_totales) as horas
	from ordenes_trabajo ot
		join areas a
			on ot.nro_area = a.nro_area
  where ot.estado = 'F'
  and ot.año_solicitud between 2017 and 2019
  group by ot.nro_area,a.area,ot.año_solicitud
 union
select a.nro_area, a.area, 2017 ,0
	from areas a
  where not exists(select *
						from ordenes_trabajo o
					  where a.nro_area = o.nro_area
					  and o.año_solicitud = 2017
					  and o.estado = 'F')
 union
select a.nro_area, a.area, 2018 ,0
	from areas a
  where not exists(select *
						from ordenes_trabajo o
					  where a.nro_area = o.nro_area
					  and o.año_solicitud = 2018
					  and o.estado = 'F')
 union
select a.nro_area, a.area, 2019 ,0
	from areas a
  where not exists(select *
						from ordenes_trabajo o
					  where a.nro_area = o.nro_area
					  and o.año_solicitud = 2019
					  and o.estado = 'F')
